...
ServerManager::ServerManager(const char *configFileName)
	: mConfig(configFileName), mServers(mConfig.getServers()), mMimeTypes(NULL)
{
...
void ServerManager::queryClientHandlers()
{
		 ...
		 clientHandlerIt != mClientHandlers.end();)
	{
		...
		if (handler.isRead() || handler.isWrite())
		{
...
ServerManager::getNewConnectionSock(Socket listenSock)
{

	Socket newSock = accept(listenSock, NULL, NULL);
	if (newSock == -1)
		throwErrnoException("getNewConnectionSock() failed"
...
void ServerManager::informClientHandlers(FDCollection &FDs)
{
...
ClientHandler &ServerManager::getClientHandler(Socket ID)
{

	ClientHandlers::iterator handler = mClientHandlers.find(ID);

	if (handler == mClientHandlers.end())
	{
		const std::string error = std::string("couldn't find client handler"
											  " with socket id: ") +
								  toString(ID);
		throw std::invalid_argument(error);
	}
